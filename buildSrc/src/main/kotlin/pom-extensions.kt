	 ...
	 withXml {
		  (((asNode()["dependencies"] as? NodeList)?.firstOrNull() as? Node)?.value() as? NodeList)?.let { dependencies ->
				...
				dependencies.map { it as Node }.forEach {
					 ...
					 val dependency = "$groupId:$artifactId"
					 it.setChildValue("version", resolvedVersions.getValue("${scope}Classpath").getValue(dependency))
				}
		  }
	 }
}
...
fun Node.setChildValue(key: String, value: String) {
	 val nodeVersion = (get(key) as NodeList)
	 ((nodeVersion.firstOrNull() as? Node) ?: appendNode(key)).setValue(value)
}
