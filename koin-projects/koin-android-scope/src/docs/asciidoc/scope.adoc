...
=== Make Android components available in scope
...
val androidModule = module {
    scope(named<MainActivity>()) {
        ...
        scoped { MainPresenter(get<MainActivity>(), get<KeyboardService>()) }

        nestedScope(named<DetailFragment>()) {
            scoped { DetailPresenter(get<DetailFragment>(), get<KeyboardService>()) }
        }
    }
}
----
...
Assuming with have two different activities:

[source,kotlin]
----
class ActivityA: AppCompatActivity() {

    private val service: ActivityScopedService by currentScopeInject()
...
To avoid repeating the same definition twice, you can extract the definition into an extension function
for the `Scope Set` api. Future changes to the definition might be limited to a single location with this approach.

[source,kotlin]
----
class ActivityScopedService(activity: AppCompatActivity)

inline fun <reified T: AppCompatActivity> ScopeSet<NestedScope>.bindAppCompatActivity() {
    factory<AppCompatActivity> { get<T>() }
}

inline fun ScopeSet<NestedScope>.activityScopedService() {
    scoped {
        ActivityScopedService(get<AppCompatActivity>())
    }
}

val androidModule = module {
    scope(named<ActivityA>()) {
        // Tell koin to use ActivityA when resolving AppCompatActivity
        bindAppCompatActivity<ActivityA>()
        activityScopedService()
    }
    scope(named<ActivityB>()) {
        // Tell koin to use ActivityB when resolving AppCompatActivity
        bindAppCompatActivity<ActivityB>()
        activityScopedService()
    }
}
----