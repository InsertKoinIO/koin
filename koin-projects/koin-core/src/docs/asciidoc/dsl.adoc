...
A definition or a module can be flagged as `eager`, to be created at start (or when you want). First set the `eager` flag on your module
or on your definition.


.Eager flag at definition level
[source,kotlin]
----
...
val myModuleB = module {
    ...
    single(eager=true) { TestServiceImp() as Service }
}
----

.Eager flag at module level
[source,kotlin]
----
val myModuleA = module {
...
val myModuleB = module(eager=true) {

    single{ TestServiceImp() as Service }
}
----

Create eager instances with `createEagerInstances()` function:
