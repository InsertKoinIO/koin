...
Let's take an example, of a Repository and Datasource. A repsotiry need a Datasource, and a Datasource can be implemented in 2 ways: Local or Remote.
...
By declaring definitions in a module with a path, all your component are then not visible from outside of this namespace.
